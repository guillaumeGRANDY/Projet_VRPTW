...
public class TabuSearch {
    ...
    public void addTabu(Tour tour)
    {
        while(this.tabuList.size()>=this.tabuSize)
        {
            this.tabuList.remove(0);
        }
        this.tabuList.add(tour);
    }


    public boolean isAllowed(Tour tour)
    {
        for(Tour tourTabu:this.tabuList)
        {
            if(tour.equals(tourTabu))
            {
                return false;
            }
        }
                ...
                for (int index = 0; index < initialTour.getRoutes().get(indexRoute2).getLivraisons().size(); index++) {
                    ...
                    if(siblingTour.tryMerge(indexRoute1,indexRoute2,index)) {
                        if (siblingTour.distance() < bestDistance && this.isAllowed(siblingTour))
                        {
                            bestTour = siblingTour;
                            bestDistance=siblingTour.distance();
                            operateurBestTour=new Pair<>(LocalSearchType.MERGE, Arrays.asList(indexRoute1,indexRoute2,index));
